...
export default function RetailAnalyticsForm({ onSubmit, initialData, isEditing }: RetailAnalyticsFormProps) {
  ...
  const handleFormSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    const completeScenario: ScenarioData = {
      ...
      netProfit: values.netProfit, // This is the target net profit from the form
      monthlyRevenue: derivedValues.totalRevenue ?? 0,
      monthlyCost: derivedValues.cogs ?? 0,
      // Use the netProfit from form values for initial netProfitAfterCommission
      netProfitAfterCommission: values.netProfit ?? 0,
      totalCommission: 0, // Initialize totalCommission
      salespeople: [], // Initialize salespeople
      breakEvenGramsPerMonth: 0, // Initialize breakEvenGramsPerMonth
      breakEvenOuncesPerMonth: 0, // Initialize breakEvenOuncesPerMonth
      profitPerGram: 0, // Initialize profitPerGram
      wholesalePricePerGram: 0, // Initialize wholesalePricePerGram
      retailPricePerGram: values.retailPriceG,
      cogs: derivedValues.cogs ?? 0,
      unitsNeeded: derivedValues.unitsNeeded ?? 0,
      totalRevenue: derivedValues.totalRevenue ?? 0,
    }
